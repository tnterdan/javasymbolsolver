...
public class CompilationUnitContext extends AbstractJavaParserContext<CompilationUnit> {
    ...
    public SymbolReference<? extends ValueDeclaration> solveSymbol(String name, TypeSolver typeSolver) {
        ...
        while (itName.contains(".")) {
            ...
            SymbolReference<org.javaparser.symbolsolver.model.declarations.TypeDeclaration> type = this.solveType(typeName, typeSolver);
            if (type.isSolved()) {
                ...
                if (importDecl.isStatic()) {
                    if (importDecl.isAsterisk()) {
                        if (importDecl.getName() instanceof QualifiedNameExpr) {
                            ...
                            org.javaparser.symbolsolver.model.declarations.TypeDeclaration importedType = typeSolver.solveType(qName);
                        ...
                        if (importDecl.getName() instanceof QualifiedNameExpr) {
                            ...
                            if (memberName.equals(name)) {
                                org.javaparser.symbolsolver.model.declarations.TypeDeclaration importedType = typeSolver.solveType(typeName);
    ...
    @Override
    public SymbolReference<org.javaparser.symbolsolver.model.declarations.TypeDeclaration> solveType(String name, TypeSolver typeSolver) {
                    ...
                    if (!importDecl.isAsterisk()) {
                        if (importDecl.getName() instanceof QualifiedNameExpr) {
                            ...
                            if (qName.equals(name) || qName.endsWith("." + name)) {
                                SymbolReference<org.javaparser.symbolsolver.model.declarations.TypeDeclaration> ref = typeSolver.tryToSolveType(qName);
                                if (ref.isSolved()) {
                                    return ref;
                                }
                            }
        ...
        SymbolReference<org.javaparser.symbolsolver.model.declarations.TypeDeclaration> ref = typeSolver.tryToSolveType("java.lang." + name);
        if (ref.isSolved()) {
            return ref;
        }
        ...
        if (isQualifiedName(name)) {
            ...
            return SymbolReference.unsolved(org.javaparser.symbolsolver.model.declarations.TypeDeclaration.class);
        }
    }
            ...
            for (ImportDeclaration importDecl : wrappedNode.getImports()) {
                if (importDecl.isStatic()) {
                    if (importDecl.isAsterisk()) {
                        if (importDecl.getName() instanceof QualifiedNameExpr) {
                            String qName = importDecl.getName().toString();
                            org.javaparser.symbolsolver.model.declarations.TypeDeclaration ref = typeSolver.solveType(qName);
                        ...
                        if (importDecl.getName() instanceof QualifiedNameExpr) {
                            ...
                            if (qName.equals(name) || qName.endsWith("." + name)) {
                                String typeName = getType(qName);
                                org.javaparser.symbolsolver.model.declarations.TypeDeclaration ref = typeSolver.solveType(typeName);
                                SymbolReference<MethodDeclaration> method = MethodResolutionLogic.solveMethodInType(ref, name, argumentsTypes, typeSolver);
                                if (method.isSolved()) {
                                    return method;
                                }
